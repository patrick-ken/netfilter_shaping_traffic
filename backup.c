...
void calculate_rwnd(connection * found, uint32_t window){
	...
	if(be > found->speed){
		...
		found->rwnd = 1 * found->rwnd + 9 * rwnd;
		//found->rwnd = (found->rwnd > window)?window:found->rwnd;
	}
	else {
...
void calculate_sbe(connection* found, uint64_t bytes_diff, uint64_t time_diff){
	...
	if(found->sbe == 0)
		...
		found->sbe = 6 * found->sbe + 4 * bytes_diff;//in b/sec
	}
}
...
unsigned int hook_func(unsigned int hooknum, struct sk_buff *skb, const struct net_device *in, const struct net_device *out, int (*okfn)(struct sk_buff *)) {
 	if(skb_is_nonlinear(skb))
		...
		if(tcp_header->ack == 1 && tcp_header->syn != 1){
			...
			if(found != NULL && (ktime_to_ns(ktime_get()) - found->last_time) >= time && flag > 0){//flag try
			...
			else if(flag < 1 && found != NULL){
				found->last_time =  ktime_to_ns(ktime_get());
				found->last_ack = found->bytes = ntohl(tcp_header->ack_seq);	
				flag = 1;
			}
		}/*end*/
		...
		if(tcp_header->syn == 1 && tcp_header->ack != 1){
			...
			if(found == NULL){
				...
				/*Initialize*/
				f->last_time = f->bytes = f->sbe = f->last_ack = f->rwnd = f->cwnd = 0;
				if(f->source_ip == inet_addr("1.1.1.1")){
					f->speed = 1000000;
					printk(KERN_INFO "I was here1\n");
				}
				else if(f->source_ip == inet_addr("1.1.1.3")){
					f->speed = 2000000;
					printk(KERN_INFO "I was here2\n");
				}
				else if(f->source_ip == inet_addr("1.1.1.4")){
					f->speed = 500000;
					printk(KERN_INFO "I was here3\n");
				}
				else f->speed = default_speed;
