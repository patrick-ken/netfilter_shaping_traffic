...
void calculate_rwnd(connection * found, uint32_t window){
	...
	if(be > speed){
		...
		/*uint64_t rwnd = window;
		do_div(rwnd, 10);
		do_div(found->rwnd, 10);
		
		found->rwnd = 8 * found->rwnd + 2 * rwnd;
		change = 1;*/
		
		uint64_t new_speed = speed * 1000;
		if(be == 0)
			be = 1;
 	...
 	if(skb_is_nonlinear(skb))
		...
		if(tcp_header->ack == 1 && tcp_header->syn != 1){
			...
			if(found != NULL && flag > 0){
				...
				uint32_t rwnd_new = rwnd_to_packet > ntohl(tcp_header->window)?ntohl(tcp_header->window):rwnd_to_packet;
			...
			if((ktime_to_ns(ktime_get()) - timer) >= time && flag > 0 && found != NULL){//flag try
				timer =  ktime_to_ns(ktime_get());
				/*time in nanosec*/
				uint64_t time_diff = timer - found->timer;
				/*write to structure*/
				found->timer = timer;
				/*time in mickrosec*/
		...
		if(tcp_header->syn == 1 && tcp_header->ack != 1){
			...
			if(found == NULL){
				...
				/*Initialize*/
				f->bytes = f->sbe = f->last_ack = f->rwnd = f->cwnd = 0;
